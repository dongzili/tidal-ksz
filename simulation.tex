...
\begin{figure}[tbp]
	...
	\caption{(Top)$P_{kSZ}$: powerspectrum of orginal kSZ signal $\Theta$, 
		...
		(Bottom) Dark solid line: correlation r between original kSZ signal 
		$\Theta$ and mock kSZ signal $\hat\Theta_{R_\parallel 15,tide}$ from foreground substracted 21cm field with $R_\parallel = 15$ Mpc/h, after tidal reconstruction
 ; blue dashed line: correlation between
		$\Theta$ and mock kSZ signal $\hat\Theta_{R_\parallel 15}$ from
		foreground substracted 21cm field with $R_\parallel = 15$ Mpc/h, without
		tidal reconstruction; 
red dotted line: correlation between
		$\Theta$ and mock kSZ signal $\hat\Theta_{R_\parallel 60}$ from
		optimal foreground substracted 21cm field with $R_\parallel = 60$ Mpc/h,
		without tidal reconstruction.} 
\label{fig:kSZ}
\end{figure}
